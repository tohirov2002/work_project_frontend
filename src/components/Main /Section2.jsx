            ...
            <section className='bg-[#2d3663] p-[50px] mt-[100px]'>
                <div className='container flex items-center justify-between mri_div'>
                    ...
                    <div className='mt-[20px]'>
                        <img className='w-full object-cover object-center max-w-[800px] rounded-xl h-[600px] mri_logo' src={Mri} alt="mri" />
                    </div>
                </div>
            </section>
                        ...
                        data.slice(0, 4).map((item) => (
                            <NavLink to={`/doctor/${item.id}`}>
                                <li key={item.id} className='hover:shadow-2xl hover:cursor-pointer shadow-xl bg-[#EDF3F6] p-5 rounded-xl h-[500px] w-[350px] flex flex-col items-center justify-center text-center'>
                                    <img className='hover:shadow-xl mt-2 rounded-3xl w-[270px] h-[270px] object-center ' src={item.image} alt={item.name} />
                                    <h1 className='text-[24px] h-[60px] mt-5 font-bold text-blue-950'>{item.name}</h1>
                                    <NavLink to={`/doctor/${item.id}`} className='hover:bg-slate-800 text-[18px] mt-6 w-[250px] text-white pt-1 pb-1 rounded-xl bg-blue-950'>{item.category_name}</NavLink>
